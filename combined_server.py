...
def load_agents():
    ...
    try:
        agents_df = pd.read_excel(AGENTS_FILE)
        agents_df.columns = [str(c).strip() for c in agents_df.columns]
        wallet_col = next((c for c in agents_df.columns if "wallet" in c.lower()), None)
        if wallet_col and wallet_col != "Wallet No.":
            agents_df = agents_df.rename(columns={wallet_col: "Wallet No."})
        agents_df["Wallet No."] = agents_df["Wallet No."].astype(str).str.strip()
        agents_df["Wallet No."] = agents_df["Wallet No."].apply(
...
def generate_qr_pdf(wallet_numbers, missing_wallets=None):
    ...
    for wallet in wallet_numbers:
        ...
        c.setFillColorRGB(0.949, 0.827, 0.078)
        c.rect(0, 0, width, height, fill=1)
        ...
        logo_path = os.path.join(BASE_DIR, 'static', 'images', 'upay_logo.png')
        if os.path.exists(logo_path):
            try:
                pil_img      = Image.open(logo_path)
                img_w, img_h = pil_img.size
                crop_h       = int(img_h * 0.72)
                cropped      = pil_img.crop((0, 0, img_w, crop_h))
                crop_buf     = io.BytesIO()
                ...
                logo_w = 3 * cm
                logo_h = 1.5 * cm
                c.drawImage(logo_reader, center_x - logo_w/2, top_y - 2.5*cm, width=logo_w, height=logo_h, preserveAspectRatio=True, mask='auto')
            except:
                pass

        c.setStrokeColorRGB(0.88, 0.88, 0.88)
        c.setLineWidth(0.5)
        c.line(card_x + 1*cm, top_y - 3.2*cm, card_x + card_w - 1*cm, top_y - 3.2*cm)

        qr_reader = ImageReader(qr_buffer)
        qr_size   = 7.5 * cm
        qr_x      = center_x - qr_size / 2
        qr_y      = card_y + card_h / 2 - 1.5 * cm
        c.drawImage(qr_reader, qr_x, qr_y, width=qr_size, height=qr_size)

        w = str(wallet)
        if len(w) == 11:   formatted = f"{w[0:3]}-{w[3:7]}-{w[7:11]}"
        elif len(w) == 10: formatted = f"{w[0:3]}-{w[3:7]}-{w[7:10]}"
        else:              formatted = w

        c.setFillColorRGB(0.106, 0.227, 0.420)
        c.setFont("Helvetica-Bold", 17)
        c.drawCentredString(center_x, qr_y - 0.9*cm, formatted)

        c.setFont("Helvetica", 9)
        c.setFillColorRGB(0.55, 0.55, 0.55)
        c.drawCentredString(center_x, qr_y - 1.6*cm, "Upay Merchant Account Number")

        c.setStrokeColorRGB(0.88, 0.88, 0.88)
        c.line(card_x + 1*cm, card_y + 3.8*cm, card_x + card_w - 1*cm, card_y + 3.8*cm)

        c.setFillColorRGB(0.106, 0.227, 0.420)
        c.setFont("Helvetica-Bold", 13)
        display_name = dba[:28] + "..." if len(dba) > 28 else dba
        c.drawCentredString(center_x, card_y + 2.8*cm, display_name)

        c.setFont("Helvetica", 9)
        c.setFillColorRGB(0.55, 0.55, 0.55)
        c.drawCentredString(center_x, card_y + 2.0*cm, "Merchant Name")
...
def search_merchants(wallet_number, merchant_type, status_filter=None,
    ...
    type_map = {"parent": "Parent", "child": "Child", "micro_merchant": "PRA", "pra": "PRA"}
    expected_type = type_map.get(str(merchant_type).lower(), "")

    filtered = agents_df[agents_df["Parent/Child"] == expected_type]

    if wallet_number and str(wallet_number).strip():
        filtered = filtered[filtered["Wallet No."].str.contains(str(wallet_number).strip(), na=False)]

    if status_filter and "Status" in agents_df.columns:
        filtered = filtered[filtered["Status"] == status_filter]
    elif not status_filter and "Status" in agents_df.columns:
        filtered = filtered[filtered["Status"] == "Active"]

    if dba_filter and str(dba_filter).strip() and "DBA" in agents_df.columns:
        filtered = filtered[filtered["DBA"].str.contains(str(dba_filter).strip(), case=False, na=False)]

    if kam_filter and str(kam_filter).strip() and "KAM" in agents_df.columns:
        filtered = filtered[filtered["KAM"].str.contains(str(kam_filter).strip(), case=False, na=False)]
